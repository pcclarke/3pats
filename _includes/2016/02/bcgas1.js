var bcgas = function() {
  ...
  var svg = d3.select("#bcgasChart").append("svg")
	...
	d3.csv("{{ site.baseurl }}/data/2016/02/bc_gas_royalties.csv", type, function(error, data) {
		x.domain(years);
    ...
    function mouseover(d) {
      ...
      d3.select("#bcgasTip").select("#bcgasVal")
  ...
  function type(d, i) {
    if (!i) years = Object.keys(d).filter(function(key) { return key !== "Natural gas royalties"; });
    var budget = {
      name: d["Natural gas royalties"],
      values: null
    };
