var onep = function() {
  ...
  var svg = d3.select("#onepChart").append("svg")
	...
	d3.csv("{{ site.baseurl }}/data/2016/03/onep.csv", type, function(error, data) {
		x.domain(years);
    ...
    function mouseover(d) {
      d3.select("#onepTip").select("#onepName")
				.text("The 1% in " + d.budget.name);
      ...
      d3.select("#onepTip").classed("hidden", false);
    }
  ...
  function type(d, i) {
    if (!i) years = Object.keys(d).filter(function(key) { return key!== "Location" && key !== "Total"; });
    var budget = {
      name: d.Location,
      total: d.Total,
      values: null
